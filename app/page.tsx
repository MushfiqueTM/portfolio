...
import { Mail, Phone, Globe, BookOpen, Briefcase, ExternalLink, X, Code, Palette, Monitor, Award } from 'lucide-react';
      ...
      >
        ...
        >
          <div className="flex flex-col md:flex-row gap-8 md:gap-12 items-center">
            ...
            >
              <motion.div 
                className="relative w-48 h-48 md:w-56 md:h-56 rounded-full overflow-hidden shadow-xl border-4"
                style={{ borderColor: 'var(--color-border)' }}
                whileHover={{ scale: 1.05, rotate: 2 }}
                transition={{ type: "spring", stiffness: 300 }}
              >
                <img src="/projects/my_picture_2.jpg" alt="Mushfique Tanzim Muztaba" className="w-full h-full object-cover" />
              </motion.div>
            </motion.div>
            <motion.div 
            ...
            >
              ...
              <motion.div 
                className="flex flex-wrap justify-center md:justify-start gap-3 sm:gap-4 md:gap-6 pt-4"
              ...
              >
                <motion.a 
                  ...
                  className="flex items-center gap-2 transition-colors min-h-[44px] px-2"
                  style={{ color: 'var(--color-body)' }}
                  whileHover={{ scale: 1.05, x: 5, color: 'var(--color-primary)' }}
                  whileTap={{ scale: 0.95 }}
                >
                  <Phone className="w-4 h-4 sm:w-5 sm:h-5" /><span className="text-xs sm:text-sm">(+852) 56014576</span>
                </motion.a>
                <motion.a 
                  ...
                  whileTap={{ scale: 0.95 }}
                >
                  <Mail className="w-4 h-4 sm:w-5 sm:h-5" /><span className="text-xs sm:text-sm break-all">mushfiquetm@gmail.com</span>
                </motion.a>
              </motion.div>
            </motion.div>
          </div>
            ...
            <div>
              ...
              >
                {['SOLIDWORKS', 'AutoCAD', 'MATLAB', 'Python', 'SQL', 'OpenCV', 'React', 'Next.js', 'TypeScript', 'JavaScript', 'HTML', 'CSS', 'Tailwind CSS', 'Framer Motion (Animation Library)', 'CSS Variables', 'Raspberry Pi', 'GPIO', 'I2C', 'PID Control', 'Computer Vision', 'Microsoft Power Platform', 'Power Apps', 'Power Automate', 'Power BI', 'Microsoft Office', 'Excel', 'PowerPoint', 'Word'].map((skill, index) => (
                  <motion.span
                    key={skill}
                    className="px-3 sm:px-4 py-1.5 sm:py-2 bg-white border border-slate-200 rounded-lg text-xs sm:text-sm font-medium text-slate-700 hover:border-[#6B4BDD] hover:text-[#6B4BDD] transition-all duration-200 shadow-sm hover:shadow-md"
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
                              {project.images.slice(0, 3).map((imagePath, imgIndex) => (
                                <div key={imgIndex} onClick={() => openLightbox(imagePath, project.images, imgIndex)} className="relative min-h-[200px] sm:min-h-[180px] rounded-lg overflow-hidden bg-[var(--color-surface)] shadow-lg hover:shadow-xl transition-shadow duration-300 group cursor-pointer flex items-center justify-center">
                                  <img src={imagePath} alt={`${project.title} - Image ${imgIndex + 1}`} className="w-full h-auto object-contain transition-transform duration-300 group-hover:scale-105" />
                                </div>
                                                              ))}
                              </div>
                              {project.images[3] && (
                                <div onClick={() => openLightbox(project.images[3], project.images, 3)} className="relative min-h-[300px] sm:min-h-[450px] rounded-lg overflow-hidden bg-[var(--color-surface)] shadow-lg hover:shadow-xl transition-shadow duration-300 group cursor-pointer flex items-center justify-center">
                                  <img src={project.images[3]} alt={`${project.title} - Image 4`} className="w-full h-auto object-contain transition-transform duration-300 group-hover:scale-105" />
                                </div>
                              )}
                            </div>
                                  ...
                                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
                                    {work.images.slice(0, 3).map((imagePath, imgIndex) => (
                                      <div key={imgIndex} onClick={() => openLightbox(imagePath, work.images, imgIndex)} className="relative min-h-[200px] sm:min-h-[180px] rounded-lg overflow-hidden bg-[var(--color-surface)] shadow-lg hover:shadow-xl transition-shadow duration-300 group cursor-pointer flex items-center justify-center">
                                        <img src={imagePath} alt={`${work.company} - Image ${imgIndex + 1}`} className="w-full h-auto object-contain transition-transform duration-300 group-hover:scale-105" />
                                      </div>
                                    ))}
                                  </div>
                                  {work.images[3] && (
                                    <div onClick={() => openLightbox(work.images[3], work.images, 3)} className="relative min-h-[300px] sm:min-h-[450px] rounded-lg overflow-hidden bg-[var(--color-surface)] shadow-lg hover:shadow-xl transition-shadow duration-300 group cursor-pointer flex items-center justify-center">
                                      <img src={work.images[3]} alt={`${work.company} - Image 4`} className="w-full h-auto object-contain transition-transform duration-300 group-hover:scale-105" />
                                    </div>
                                  )}
                                </div>
                              ) : (
