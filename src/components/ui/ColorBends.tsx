...
// Helper to convert hex to RGB vector
...
export default function ColorBends({
  ...
  useEffect(() => {
    ...
    const colorArray = (colors || []).filter(Boolean).slice(0, MAX_COLORS).map(hexToVec3);
    const uColorsArray = Array.from({ length: MAX_COLORS }, (_, i) => 
      i < colorArray.length ? colorArray[i] : new THREE.Vector3(0, 0, 0)
    );
    ...
    const material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        uColorCount: { value: colorArray.length },
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < MAX_COLORS; i++) {
      ...
      if (i < colorArray.length) vec.copy(colorArray[i]);
      else vec.set(0, 0, 0);
    }
    material.uniforms.uColorCount.value = colorArray.length;
